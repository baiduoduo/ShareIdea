...
public class IdeaInfoDAO implements IIdeaInfoDAO{
	...
	public String getFriendIdeaMsg(int userId,int pageNum,int pageSize,String sortName){
		...
		try{
			...
			query = query + "(idea.ideaCategory like '%"+sortName+"%') order by idea.ideaId";
	...
	public String getIdeainfo(int ideaId){
		...
		try{
			...
			if(idea.size()>0){
				for(Ideainfo c:idea){
					result = c.getIdeaTitle() + "," + c.getNickName() + "," + c.getUpdateTime() + "," + c.getIdeaCommentNo() + "," + c.getIdeaComtent()+","+c.getUserbaseinfo().getHeadimage()+","+c.getIdeaId()+","+c.getIdeaNum()+","+c.getUserbaseinfo().getUserId();
				System.out.println(result);
				}
			}
		}catch(Exception e){
	...
	public String getIdeaCodeMsg(int ideaid){
		String HQL = "select new Codeinfo(code.nickName,code.codeName, code.userbaseinfo,code.codeId) from Codeinfo as code where code.ideainfo.ideaId = "+ideaid;
		Session session = null;
		try{
			session = HibernateTool.getSession();
			session.getTransaction().begin();
...
/**
	...
	private String getIdeaCodeMsgToString(List<Codeinfo> code) {
		...
		if(code.size()>0){
			for(Codeinfo c :code){
				result+="{\"codeName\":\""+c.getCodeName()+"\",\"userId\":\""+c.getUserbaseinfo().getUserId()+"\",\"codeAuthor\":\""+c.getNickName()+"\",\"codeId\":\""+c.getCodeId()+"\"},";
			}
			result = result.substring(0, result.length()-1)+"]}";
		}else{
			result="{}";
	...
	public String showOtherSpaceIdeaMsg(int userId,int otherUserId){
		String HQL = "select new Ideainfo(idea.ideaId,idea.ideaTitle,idea.ideaComtent,idea.nickName,idea.ideaClickNo,idea.ideaCategory) from "
				+ "Ideainfo as idea , Ideaauthority as it where idea.userbaseinfo.userId="+otherUserId+" and (idea.publicAuthority = 1 or idea.publicAuthority = 0 and it.ideainfo.ideaId=idea.ideaId and it.userbaseinfo.userId="+userId
				+") group by idea.ideaId,idea.ideaTitle,idea.ideaComtent,idea.nickName,idea.ideaClickNo,idea.ideaCategory";
		Session session = null;
		String result = "{";
		try{
				session = HibernateTool.getSession();
