...
public class UserbaseinfoDAO implements IUserbaseinfoDAO {
	...
	public boolean insertUser(Userbaseinfo user){
		...
		int id = getId(user.getNickName());
		return initAddress(id);
	}
	public int getId(String nickName){
		return getUserId(nickName)==-1?null:getUserId(nickName);
	}
	...
	public Userbaseinfo getUserAllMsgById(int id) {
		...
		try{
			String sql = "select *,replace(userIntroduct,char(10)+char(13),'<br>') from Userbaseinfo where userId ="+id;
			session = HibernateTool.getSession();
			session.getTransaction().begin();
...
//			    user=  (Userbaseinfo)q.list().get(0);
			List list = session.createSQLQuery(sql).addEntity(Userbaseinfo.class).list();
			Iterator it = list.iterator();
			if(it.hasNext()){
				user = (Userbaseinfo) it.next();
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	public String getPasswordByAccount(String account) {
		...
		String loginModel;
		 ...
		 HQL = "select new Userbaseinfo(user.userId, user.userPassword, user.nickName) from Userbaseinfo as user where user."
				 + mqStr + "=:account";
		 Session session = HibernateTool.getSession();
		 session.getTransaction().begin();
		 Query ql = session.createQuery(HQL);
		 ql.setString("account", account);
		 if(ql.list().isEmpty()){
			 return null;
		 }
		 Userbaseinfo user = (Userbaseinfo) ql.list().get(0);
		 loginModel = user.getUserPassword() + "," + user.getUserId() +  "," + user.getNickName();
	...
	public boolean updateUserMsg(Userbaseinfo userInfo,int id) {
		String HQL = "update Userbaseinfo as user set userSex="+userInfo.getUserSex()+",userProf='"+userInfo.getUserProf()+
				"',userAddress='"+userInfo.getUserAddress()+"',userIntroduct='"+userInfo.getUserIntroduct()+"' where userId="+id+"";
		Session session=null;
		int update=-1;
		try{
			session = HibernateTool.getSession();
	...
	public int getUserScore(int userId) {
		String getUserScore = "select user.userScore from Userbaseinfo as user where user.userId="+userId;
		Session session=null;
		int userScore = 0;
		try{
			session = HibernateTool.getSession();
			session.getTransaction().begin();
			Query q = session.createQuery(getUserScore);
			userScore = (int)q.list().get(0);
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			HibernateTool.closeSession(session);
