...
public class UploadIdeaAction extends ActionSupport {
	...
	public String insertIdeaInfo() throws Exception{	
		...
		if("上传策略".equals(ideaUploadMode.getUploadMethod())){
			...
			//TODO 在保存数据之前需要将回车替换为<br>标签
			ideaBaseInfo.setIdeaComtent(ideaUploadMode.getIdeaContent().replace("\r\n", "<br>"));
			...
			userDao.updateNo("userIdeaNo", getUserId(), true,1);
			userDao.updateNo("userScore", getUserId(), true,3);
			//保存关键字
			...
			if("所有人".equals(authority)){
				...
				ideaBaseInfo.setPublicAuthority(0);
	...
	private String saveCodeFile(HttpSession session,int maxNum) throws IOException{
		...
		DAOFactory.getIUserbaseinfoDAOInstance().updateNo("userScore", getUserId(), true,3);
